...
export class Rect2D{
	...
	setHitbox(hitbox){
		this.hitbox = hitbox;
		this.hitbox.hitboxID = "HBOX#ID"+Object.keys(this.arcsci.colliderQueue).length.toString();
		this.arcsci.colliderQueue[this.hitbox.hitboxID] = this.hitbox;
	}
	draw(){
		...
		if(this.hitbox.isObjectColliding()){
			this.x = 0;
			this.y = 0;
			wX = 0;
			wY = 0;
...
export class CollisionHitbox2D{
	...
	isObjectColliding(){
		for(let i = 0; i < Object.keys(this.arcsci.colliderQueue).length; i++){
			...
			if(this.hitboxID != "HBOX#ID"+i.toString()){
//				console.log(
...
export class DetectionHitbox2D{}
