...
export class Rect2D{
	...
	setHitbox(hitbox){
		this.hitbox = hitbox;
		this.arcsci.colliderQueue.push(this.hitbox);
	}
	checkForCollision(){
		...
		let tempWorldX = this.x;
		let tempWorldY = this.y;
...
export class CollisionHitbox2D{
	...
	isObjectColliding(){
		for(let i = 0; i < this.arcsci.colliderQueue.length; i++){
			let colObj = this.arcsci.colliderQueue[i];
